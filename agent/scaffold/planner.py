...
class LazyLocalModelPlanner:
    ...
    def next_plan(self, context: PlanningContext) -> PlanDecision:
        ...
        system = (
            ...
            "numbered view. For edits, pass the exact SHA-256 from the latest view_window into "
            "checked_edit; treat a stale-SHA or static-parse rejection as authoritative and re-read "
            "instead of forcing the change. After mutation, use the reopened checked_edit view and "
            "run_check to prove syntax/tests before finish. Prefer this search->view->edit->check "
            "protocol over broad read_file/search_text/apply_patch calls. "
            "Repository Distiller remains available for structure: use rank_relevant_files when a "
            "file location is unknown, repo_tree for architecture, repo_skeleton for compact signatures, "
            "and inspect_symbol for focused source plus references. Legacy broad tools are fallback "
            "interfaces when the compact ACI cannot express the required operation. "
