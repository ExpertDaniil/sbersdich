...
## A-13: эффективность контекста

Сравнение использует одинаковые истории из 24 событий для режимов `audit`,
`fix`, `forensics` и `general`. Базовый вариант получает все 24 события,
рабочий — последние 6. Для каждого запроса измеряется точный размер тела в
байтах, проверяется сохранение последнего важного наблюдения и совпадение
выбранного действия.
...
Обычный прогон не использует сеть: ответ формирует детерминированная подмена,
которая завершает задачу только при наличии последнего важного наблюдения.
Для дополнительной проверки на настоящей локальной модели доступны те же
сценарии и настройки через `--live`; инструменты при этом не запускаются.
В таком прогоне отчёт отдельно сохраняет фактические входные токены из ответа
модели. Действия сравниваются после подстановки необязательных значений по
умолчанию, поэтому `list_files(path=".")` и та же команда с явно указанными
`max_depth=6`, `max_entries=200` считаются равными.

Если внешний адрес временно вернул 429/503 или не успел ответить, прогон можно
продолжить по тому же файлу. Уже успешные сценарии будут взяты из отчёта, а
повторятся только неуспешные; между запросами выдерживается заданная пауза:
...
python3 -m evaluation.context_efficiency \
  ...
  --resume \
  --cooldown-seconds 20 \
  --output evaluation/results/a13_gemini35.json
```

Во время такого прогона названия текущих сценариев и паузы выводятся сразу,
поэтому длительное ожидание ответа не выглядит как зависание.

Код возврата `0` означает совпадение действий во всех сценариях, сохранение
важного наблюдения и сокращение не менее чем на 45%; `1` — критерий не выполнен;
`2` — ошибка настройки модели или записи отчёта. Подробности:
[`../docs/A13_REPORT.md`](../docs/A13_REPORT.md).
